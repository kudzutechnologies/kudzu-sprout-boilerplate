...
CayenneLPP::CayenneLPP(uint8_t* buffer, uint8_t size) : buffer(buffer), maxsize(size)
{
	cursor = 0;
}

void CayenneLPP::setBuffer(uint8_t* buffer, uint8_t size)
{
	this->buffer = buffer;
	this->maxsize = size;
	cursor = 0;
...
// Fuel Gauge
uint8_t CayenneLPP::addFuelGauge(uint8_t channel, uint16_t stateOfCharge, uint16_t remainingCapacity, uint16_t voltage, uint16_t averageCurrent, uint16_t temperature, uint16_t stateofHealth)
{
	if ((cursor + LPP_FUELGAUGE_SIZE) > maxsize)
	{
		return 0;
	}

	uint8_t id = 3 + channel * 6;
	addAnalogInput(id+0, (float)(int)stateOfCharge);
	addAnalogInput(id+1, ((float)(int)remainingCapacity)/100);
	addAnalogInput(id+2, ((float)(int)voltage)/100);
	addAnalogInput(id+3, ((float)(int)averageCurrent)/100);
	addAnalogInput(id+4, ((float)(int)temperature)/100);
	addAnalogInput(id+5, ((float)(int)stateofHealth)/100);
