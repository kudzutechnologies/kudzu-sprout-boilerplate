...
#define LPP_FUELGAUGE 100			// Generic Sensor
...
#define LPP_FUELGAUGE_SIZE 				LPP_ANALOG_INPUT_SIZE * 6

class CayenneLPP
{
public:
	CayenneLPP(uint8_t* buffer, uint8_t size);
	void setBuffer(uint8_t* buffer, uint8_t size);
	...
	uint8_t addFuelGauge(uint8_t channel, uint16_t stateOfCharge, uint16_t remainingCapacity, uint16_t voltage, uint16_t averageCurrent, uint16_t temperature, uint16_t stateofHealth);
